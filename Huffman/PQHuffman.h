...
template<typename Data, typename Priority>#ifndef PQHUFFMAN_H
#define PQHUFFMAN_H
#include <vector>
#include <iostream>
#include <iomanip>
...
public:
	...
	void sortDown(unsigned int a);
	void sortUp(int a);
	void push(Data data, Priority priority);
	bool is_empty();
	void dump(item<unsigned char, int>* temp, int i);
	item<Data, Priority>* pop();
	std::pair<int, unsigned long long> symbols[256];
	std::vector<item<Data, Priority>*> heap;
	bitStreams& bitsBytes_() { return bitsBytes; }
private:
	bitStreams bitsBytes;
...
inline void Heap<Data, Priority>::buildCStable(item<unsigned char, int>* temp)
{
	if (!temp->right() && !temp->left()) {
		temp->visited() = true;
		symbols[temp->data()] = make_pair(bitsBytes.numBitsIn(), bitsBytes.bufferOut());
		bitsBytes.bufferOut() = '\0';
		bitsBytes.numBitsIn() = 0;
		return;
	}
	if (!temp->data() && temp->visited() == false) {
		if (temp->left() && temp->left()->visited() == false) {
			bitsBytes.bufferOut() = bitsBytes.bufferOut() << 1;
			bitsBytes.bufferOut() = bitsBytes.bufferOut() | 0;
...
{
	bitsBytes.numBitsOut() = symbols[temp].first;
	bitsBytes.bufferOut() = symbols[temp].second;
	bitsBytes.location() = bitsBytes.numBitsOut() - 1;
	while (bitsBytes.location() >= 0) {
		bitsBytes.putBit(bitsBytes.getBit());
		--bitsBytes.location();
	}
}

template<typename Data, typename Priority>
inline void Heap<Data, Priority>::sortDown(unsigned int a)
{
